...
import { View, Text, StyleSheet, Pressable, TextInput, Platform, TouchableOpacity, SafeAreaView, Modal, Dimensions, Alert } from "react-native";
import React, { useState, useEffect } from 'react';
import { useSelector, useDispatch } from 'react-redux';
import axios from 'axios';
import IconAntDesign from 'react-native-vector-icons/AntDesign';
import { updateAsset1 } from '../redux/assets'
import { addEX1 } from '../redux/assets'

function AddFComp(props) {
    ...
    const [asset1, setAsset1] = useState({ _id: props.data._id, rent: props.data.rent, funding: props.data.funding, adress: props.data.adress, size: props.data.size, furniture: props.data.furniture, expenses: props.data.expenses, image: props.data.image, rentF: props.data.rentF, contractImage: props.data.contractImage })
    ...
    const getDate = async () => {
        ...
        const index = asset1.expenses.length - 1
        if (asset1.expenses[index].months == month) {
            if (newEx.name == '' || newEx.price == 0) {
                Alert.alert('נא למלא את כל השדות')
            } else {
                const data = { ...asset2 }
                const newexpenses = [...asset2.expenses]
                newexpenses[index] = {
                    ...newexpenses[index],
                ...
                dispatch(addEX1({ id: data._id, newEx: newEx }))
        ...
        useEffect(() => {
            ...
            setNewexpenses(asset1.expenses)
        }, [asset])
